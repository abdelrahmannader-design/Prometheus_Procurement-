...
class StatTile(Card):
    ...
    def destroy(self):
        # A scheduled KPI font-fit callback can outlive the widget during
        # theme/navigation rebuilds. Cancel it before Tcl destroys the command
        # so teardown stays quiet and deterministic.
        job = getattr(self, "_fit_after", None)
        if job is not None:
            try:
