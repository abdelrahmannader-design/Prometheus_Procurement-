...
class StressTests(unittest.TestCase):
    ...
    def test_finance_carry_is_included_in_stress_base(self):
        inputs = {
            "commodity": "CORN",
            "cbot": 530.75,
            "fx": 51.1840,
        ...
        self.assertIsNone(result["error"])
        self.assertAlmostEqual(result["base"]["saving_egp_mt"], -220.00, places=2)
        self.assertAlmostEqual(result["base"]["carry_usd_mt"], 5.130075, places=4)
        self.assertAlmostEqual(result["be_fx"], 50.4119, places=4)
        self.assertAlmostEqual(result["be_cbot"], 520.03, places=2)

    def test_stress_base_reconciles_to_single_deal_direct_saving(self):
        decision_inputs = {
            "comm_meta": {"type": "CBOT", "locked_factor": CORN_FACTOR},
            "cbot": 530.75,
            "premium_usd_mt": 180.0,
            "fx": 51.1840,
            "qty_mt": 4_000,
            "finance_days": 30.0,
            "interest_rate": 22.0,
            "local_egp_mt": 14_600.0,
            "supplier_direct_egp_mt": 235.0,
            "supplier_indirect_egp_mt": 365.0,
        }
        single = compute_decision(decision_inputs)
        stress = run_stress_test({
            "commodity": "CORN",
            "cbot": decision_inputs["cbot"],
            "fx": decision_inputs["fx"],
            "premium_cents": decision_inputs["premium_usd_mt"],
            "qty_mt": decision_inputs["qty_mt"],
            "local_egp_mt": decision_inputs["local_egp_mt"],
            "fees_egp_mt": decision_inputs["supplier_direct_egp_mt"],
            "finance_days": decision_inputs["finance_days"],
            "interest_rate": decision_inputs["interest_rate"],
            "premium_locked": True,
        })
        self.assertAlmostEqual(
            stress["base"]["saving_egp_mt"],
            round(single["direct_saving_egp_mt"], 2),
            places=2,
        )

    def test_contract_reconciliation_uses_canonical_all_in_fees(self):
        # Regression: contract 460/20048300 must agree with Portfolio/Open
        # Contracts instead of showing a false saving because freight/clearance
        # were omitted from the Single Deal Calculator.
        out = compute_decision({
            "comm_meta": {"type": "NONCBOT"},
            "import_usd_mt": 285.92,
            "fx": 50.45,
            "qty_mt": 3_000,
            "finance_days": 0.0,
            "interest_rate": 23.0,
            "local_egp_mt": 15_097.0,
            "supplier_direct_egp_mt": 245.63,
            "supplier_indirect_egp_mt": 200.0,
            "canonical_landed_fees_egp_mt": 245.63 + 200.0 + 485.1042,
        })
        expected_own = 285.92 * 50.45 + 245.63 + 200.0 + 485.1042
        self.assertAlmostEqual(out["own_after_egp_mt"], expected_own, places=4)
        self.assertAlmostEqual(out["direct_own_after_egp_mt"], expected_own, places=4)
        self.assertAlmostEqual(out["saving_egp_mt"], 15_097.0 - expected_own, places=4)
        self.assertLess(out["saving_egp_mt"], 0)

    def test_decision_engine_uses_finance_in_base_case(self):
        ...
        self.assertAlmostEqual(result["base_saving_mt"], -220.00, places=2)
