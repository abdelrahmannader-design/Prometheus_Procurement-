...
def calculate_inventory_scenario(*, current_remaining_mt, avg_inventory_cost_egp_mt,
    ...
    ``purchase_source`` makes local purchasing a first-class scenario rather
    than treating the local market only as a benchmark. IMPORT compares the
    CBOT/import purchase cost against local all-in. LOCAL compares the local
    all-in purchase against the same live import benchmark. Positive
    ``scenario_saving_egp_mt`` therefore always means the selected source is
    ...
    if cif is not None and fx_v is not None and fx_v > 0 and effective_freight is not None:
        import_cost = cif * fx_v + effective_freight + other_fees

    source = str(purchase_source or "IMPORT").strip().upper()
    if source.startswith("LOCAL"):
        source = "LOCAL"
        purchase_cost = local_all_in
        alternative_cost = import_cost
        comparison_label = "Local purchase vs CBOT/import"
    else:
        source = "IMPORT"
        purchase_cost = import_cost
        alternative_cost = local_all_in
        comparison_label = "CBOT/import vs local purchase"

    saving_mt = None
    if alternative_cost is not None and purchase_cost is not None:
        saving_mt = alternative_cost - purchase_cost
    ...
    post_purchase_avg = None
    if purchase_cost is not None and (current_qty + purchase_qty) > 0:
        current_value = current_qty * (avg_cost or 0.0)
        if avg_cost is None and current_qty > 0:
            post_purchase_avg = None
        else:
            post_purchase_avg = (current_value + purchase_qty * purchase_cost) / (current_qty + purchase_qty)
    post_purchase_local_edge = (
        local_all_in - post_purchase_avg
        if local_all_in is not None and post_purchase_avg is not None else None)
    return {
        ...
        "import_benchmark_cost_egp_mt": import_cost,
        "scenario_purchase_cost_egp_mt": purchase_cost,
        "alternative_cost_egp_mt": alternative_cost,
        ...
        "post_purchase_avg_cost_egp_mt": post_purchase_avg,
        "post_purchase_local_edge_egp_mt": post_purchase_local_edge,
    }
